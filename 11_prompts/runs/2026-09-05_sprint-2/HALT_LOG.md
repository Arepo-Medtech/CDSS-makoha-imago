...
Byte-count fixed point (ledger2.py, final run after every review fix): quoted = disk for HARDEN-1.2 (76468), HARDEN-3.2 (86892), 00_inventory_v1.3.txt (24257), INDEX-09.1 (4155), GLOSSARY.md (12063) — verified by `stat -f%z` at seal; numbers here are the seal values.
| H-12 | Copilot review on PR #16 | AGENTS.md "How work lands" read literally forbids a sprint's deliverables outside the run directory, contradicting the A-004 precedent this sprint follows | GOVERNANCE-TEXT | AGENTS.md (outside the 00_–11_ law) sentence tightened: survey runs = run directory only; sprint runs = record + new files beside retained files; recorded in A-010 |
| H-13 | Copilot review on PR #16 | HALT_LOG fixed-point line quoted GLOSSARY.md at 11410 B after two later edits (final 12064+) | STALE-EVIDENCE | line regenerated from the inventory at seal (above); GLOSSARY `:165` fragment fixed to `l.165` |
