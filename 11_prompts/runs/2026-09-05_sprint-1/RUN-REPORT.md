# RUN-REPORT — sprint-1 (2026-09-05): closing the survey-2 Build-Spec Queue

Run: `11_prompts/runs/2026-09-05_sprint-1/` · Branch: `sprint-1-build-spec-queue` (from `main` 6aae5d6) · Executor: Claude Code (desktop session) · Mandate: the owner's request of 5 Sep 2026 to run a sprint that closes the Build-Spec Queue · Status: **built and verified; not merged** — `main` accepts changes only by pull request (README).

## 0. Append-only proof (law 1) — read this first

```
CHECKSUMS_BEFORE.txt: 292 files (main, before any write)
CHECKSUMS_AFTER.txt:  342 files
pre-existing files whose hash changed: 1
< ae7261eb67775f52afceb134f2dd32aa659dfbf9dc80a485f81700acb44561a2  ./00_MANIFEST.md
00_MANIFEST.md prefix check: sha256(first 19282 bytes of the new file) = ae7261eb67775f52afceb134f2dd32aa659dfbf9dc80a485f81700acb44561a2
                             sha256(main:00_MANIFEST.md)              = ae7261eb67775f52afceb134f2dd32aa659dfbf9dc80a485f81700acb44561a2
                             → IDENTICAL — appended only (A-004)
```
...
| Measure | Value |
|---|---|
| New files outside run directories | 37 |
| Run directories added | 2 (`2026-09-05_sprint-1`, `2026-09-05_primer-0` partial) |
| Files in HARDEN-1.1 scope (tree excl. .DS_Store, .git, runs) | 270 — all on disk at seal: 270/270 |
| HARDEN-1.1 rows | 275 (v1.0 ids 0–73 resolved + 198 new); acceptance set-equality: 0 missing / 0 extra → PASS |
| HARDEN-3.1 tasks | 276; every v1.0 T id present once (73/73); every scoped file has exactly one task |
...
| Diagrams | mermaid mermaid 10.9.8 via jsdom 24.1.3 (node v20.20.2): 16/16 PASS; source↔inline identity 9/9 |
| CC-5 field presence (OPS-1.1) | 33/33 steps |
| Reference check (00_, 04_–10_) | {"path_refs_checked": 605, "anchor_refs_checked": 348, "unresolved_paths": 38, "resolved_by_basename_only": 14, "unresolved_anchors": 3} — unresolved paths are all external-pack / glob / future-output / (at check time) not-yet-written INDEX files; unresolved anchors are the two carried v2 `MT2 §7.4` defects (DEF-003) — see refcheck_output.txt |

## 2. Queue closure — every survey-2 row with weight ≥ 3, plus the weight-2 rows acted on

| Row | W | Folder | Deliverable | Outcome | Evidence |
|---|---|---|---|---|---|
...
Summary: **30 BUILT · 2 CLOSED BY A-003 · 1 DRAFTED (after-decision) · 4 REMAIN (decision-gated) · 10 HUMAN-ONLY rows unchanged (they are decisions).** No CLAUDE-CODE-EXECUTABLE-NOW row remains open.
