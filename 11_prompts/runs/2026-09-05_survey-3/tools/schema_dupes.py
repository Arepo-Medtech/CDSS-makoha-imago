...
JSON Schema vocabulary keys (required, properties, description, …) are structural, not values, and are excluded from the pinned-string census (Copilot review on PR #14, accepted)."""
...
JSONSCHEMA_KEYS={'required','properties','minLength','maxLength','description','additionalProperties','$comment','$schema','$id','title','type','enum','pattern','items','minimum','maximum','allOf','anyOf','oneOf','const','minItems','$defs','$ref','if','then','not','format','default','examples','uniqueItems','propertyNames','patternProperties'}
for f in fs:
...
dup_strings=sorted([{"string":k,"files":len(v),"examples":sorted(v)[:3]} for k,v in strings.items() if len(v)>=3 and not k.startswith(('http','#')) and k not in JSONSCHEMA_KEYS],key=lambda x:-x["files"])[:40]
