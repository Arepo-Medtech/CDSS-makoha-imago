...
for p in pages:
    ...
    rows.append({"page":p,"bytes":len(t.encode()),"hex_colours":len(hexes),"px_rem_values":len(sizes),"font_families":sorted(fonts),"media_queries":mq,"external_stylesheets":ext,"style_blocks":styleblocks,"hex_list":sorted(hexes)})
