...
**Governance text aligned (outside the 00_–11_ law).** `AGENTS.md` "How work lands" said survey *and* sprint runs leave the diff outside the run directory empty; sprint-1 (A-004, 37 files) and this sprint do not, by design — a sprint builds deliverables. The sentence now distinguishes survey runs (run directory only) from sprint runs (run record + new files beside retained files, bookends proving no pre-existing change but the manifest and root governance). Found by the Copilot review on PR #16; changed on the branch.
